...
from clustering_ui import Ui_MainWindow
from os.path import basename, dirname
import ftir_function as ff
import numpy as np
...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def Load_chose(self):
        if self.comboBoxSingMul.currentText() == 'Multiple Files':
        ...
        elif self.comboBoxSingMul.currentText() == 'Single File':
            self.LoadSpec()
    ...
    def readfile(self,fileName):
        ...
        try:    
            sxsy, self.p ,self.wavenumber, self.sp = ff.readmat(fileName)
            self.index = np.random.randint(0,int(sxsy*sxsy),(10))
            self.clear_all()
            self.lock_un(True)
            ...
            self.lineEditWidth.setText(str(sxsy))
            self.lineEditHeight.setText(str(sxsy))
    ...
    def ClusteringCal(self):        
        ...
        if method == 0:
            self.clus = KMeans(n_clusters = nclus, random_state=0).fit(self.df_conc)
        else:
            self.clus = MiniBatchKMeans(n_clusters = nclus, random_state=0).fit(self.df_conc)
        
            
        self.mapping = np.reshape(self.clus.labels_,(int(self.lineEditWidth.text()),
                                           int(self.lineEditHeight.text())))
    ...
    def ExpandSpectra(self):
         if self.comboBoxVisualize.currentText() == 'Spectra and White Light Image':    
            plt.close(0)
            plt.figure(0)
...
#            self.labelWavenumber.setEnabled(False)
...
#            self.labelWavenumber.setEnabled(True)
...
#        global projection
    ...
    def Cvisualize (self):      
        ...
        if 'component_' in self.comboBoxVisualize.currentText():
            ...
            self.datap = self.df_conc.iloc[:,val].as_matrix()
            
#            global component
            self.component = np.reshape(self.datap,(int(self.lineEditWidth.text()),
                                           int(self.lineEditHeight.text())))
            self.plotMultiVisual.canvas.ax.clear()
...
#            global datas
            self.datas = self.df_spec.iloc[:,val].as_matrix()
            self.plot_specta.canvas.ax.clear()
...
#                new_label = [item for item, count in collections.Counter(label).items() if count > 0]
#                
